...
class WriteOutline(dspy.Module):
    """Generate the outline for the argumentative essay."""
...
class WritePageOutline(dspy.Signature):
    ...
    outline = dspy.OutputField(prefix="Write the essay outline:\n", format=str)
...
class WritePageOutlineFromConv(dspy.Signature):
    """Improve an outline for an argumentative essay. You already have a draft outline that covers the general information. Now you want to improve it based on the information learned from an information-seeking conversation to make it more informative.
        Here is the format of your writing:
        1.  Use "#" Title" to indicate section title (Introduction, Main Sections or Conclusion), "##" Title" to indicate subsection title (Subsections within each Main Section), 
           "###" Title" to indicate subsubsection title (Subtopics within each Subsection), and "####" Title" to indicate details (Supporting Details within each Subtopic).
    ...
    outline = dspy.OutputField(
        prefix='Write the argumentative essay outline (Use "#" Title to indicate section title, "##" Title to indicate subsection title, ...):\n',
        format=str
    )
